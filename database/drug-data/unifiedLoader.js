...
import { CONSOLIDATED_MAPPINGS } from '../../server/api-routes/icd10/mapping-consolidator.js';
...
export async function loadUnifiedMappings() {
  ...
  try {
    // Priority 1: Use code-based consolidated mappings (Single Source of Truth)
    // This ensures we always use the latest mappings including UAE data
    if (CONSOLIDATED_MAPPINGS && Object.keys(CONSOLIDATED_MAPPINGS).length > 0) {
      const normalizedMappings = {};
      for (const [key, val] of Object.entries(CONSOLIDATED_MAPPINGS)) {
        normalizedMappings[normalize(key)] = val;
      }

      mappingsCache = normalizedMappings;
      cacheTimestamp = Date.now();
      console.log(
        `[UnifiedLoader] Loaded ${Object.keys(mappingsCache).length.toLocaleString()} mappings from consolidated code`
      );
      return mappingsCache;
    }
