...
-- Create drugs table (Removed: Using drugs table from drug_schema.sql)
...
CREATE INDEX IF NOT EXISTS idx_codes_billable ON codes(is_billable);
CREATE INDEX IF NOT EXISTS idx_codes_header ON codes(is_header);
CREATE INDEX IF NOT EXISTS idx_chapters_code ON chapters(code);
-- CREATE INDEX idx_drugs_name ON drugs(name); -- Removed: Duplicate index, already in drug_schema.sql
CREATE INDEX IF NOT EXISTS idx_insurance_requirements_code ON insurance_requirements(code_id);
...
CREATE VIEW IF NOT EXISTS drug_code_search AS
SELECT 
    d.id as drug_id,
    d.drug_name as drug_name, -- Fixed column name
    c.code as icd_code,
    c.short_description,
    c.long_description,
